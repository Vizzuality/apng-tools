...
int APNGOpt::load_apng(void *buffer, long size, std::vector<APNGFrame> &frames, unsigned int &first, unsigned int &loops)
{
    FILE *f;
...
size_t APNGOpt::save_apng(long & buffer_ptr, std::vector<APNGFrame> &frames, unsigned int first, unsigned int loops, unsigned int coltype, int deflate_method, int iter)
{
    FILE *f;
    unsigned int i, j, k;
    ...
    if ((f = open_memstream(&ptr, &fileSize)) != 0)
    {
